...
export default function ContactFormCard() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!FORMSPREE_ID) { setStatus('err'); setMsg('Missing Formspree ID.'); return; }
    setStatus('loading'); setMsg('');
    const form = e.currentTarget;
    const data = new FormData(form);
    if (data.get('company')) { setStatus('ok'); form.reset(); return; }
    data.set('schedule_demo', demo ? 'yes' : 'no');

    const res = await fetch(`https://formspree.io/f/${FORMSPREE_ID}`, {
      method: 'POST', headers: { Accept: 'application/json' }, body: data
    });
    if (res.ok) { setStatus('ok'); setMsg('Thanks! We’ll get back to you shortly.'); form.reset(); setDemo(false); }
      ...
      >
        ...
        <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
          ...
          <div className="c4-float">
            ...
            <label htmlFor="first_name"><span className="c4-label-text">First name</span></label>
          </div>
          <div className="c4-float">
            <input id="last_name" name="last_name" placeholder=" " />
            <label htmlFor="last_name"><span className="c4-label-text">Last name</span></label>
          </div>

          <div className="c4-float">
            <input id="email" name="email" type="email" placeholder=" " required />
            <label htmlFor="email"><span className="c4-label-text">Work email</span></label>
          </div>
          <div className="c4-float">
            <input id="company_size" name="company_size" placeholder=" " />
            <label htmlFor="company_size"><span className="c4-label-text">Company size</span></label>
          </div>

          <div className="c4-float">
            <input id="phone" name="phone" type="tel" placeholder=" " />
            <label htmlFor="phone"><span className="c4-label-text">Phone</span></label>
          </div>
          <div className="c4-float">
            <input id="topic" name="topic" placeholder=" " />
            <label htmlFor="topic"><span className="c4-label-text">opic</span></label>
          </div>

          <div className="c4-float c4-float--textarea md:col-span-2">
            <textarea id="message" name="message" placeholder=" " />
            <label htmlFor="message"><span className="c4-label-text">Message</span></label>
          </div>
          ...
          {status !== 'idle' && (
            <div className="md:col-span-2 -mt-1 text-sm">
              <span className={
                ...
                status === 'err' ? 'text-red-400' : 'text-white/70'
              }>{msg}</span>
            </div>
          )}
        </form>
